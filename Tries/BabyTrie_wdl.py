...
class BabyTrie:#baby version of Trie
    ...
    def isinTrie(self,sen):
        ...
        for i in range(len(lst)):
            ptr-self.root
            if lst[i] not in ptr.children:
                for children in ptr.children:
                    dl_score=dl(children,lst[i],similarity=True,deleteWeight=2,insertWeight=3,replaceWeight=2)
                    print(dl_score)
                    if dl_socre>=0.8:
                        dl_match=children
                        proceed=True
                        rstart=i
                        ptr=ptr.children[dl_match]
                        break
            elif lst[i] in ptr.children:
                rstart=i
                ptr=ptr.children[lst[i]]
