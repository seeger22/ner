...
class BabyTrie:#baby version of Trie
    ...
    def isinTrie(self,sen):
        ...
        plst=re.findall(r"[\w']+|[.,!?;]", new_sen)#premature list used for clean-up CHANGE IF SOME SYMBOLS NEEDS TO BE CONSIDERED FROM LOGS
        p=re.compile(r"^(\w+)(\'\w+)$")
            ...
            for elem in dic[cat]:
                ...
                if ('name' in record and record['name'] is not None):
                    ...
                    if len(lst[0])==2:#sw hotel <-> s.w. hotel
                        acron=lst.pop(0)#treated as acron
                        lst.insert(0,'.')
        ...
        for cat in spdic.keys():
            for entry in spdic[cat]:
                if (entry['id'] is not None) and (entry['name'] is not None):
                    id=entry['id']
                    new_name=entry['name'].lower().strip()
                    if new_name == cat:
                        print('exceptional entity---------------------')
                        continue
                    lst=[]#final list that is used to insert into bt
...
def main():
    ...
    #/content/drive/MyDrive/Research/ner/alexa-with-dstc10-track2-dataset/task1/data/db.json
    ...
    #print(bt.root.children['h'].children['.'].children['k'].children['.'].children['fusion'].children['restaurant'].entity)

    kb.close()

    test="holy shit taylor hotel of san francisco works now!"
    wordlst,indlst=bt.isinTrie(test)
    print(wordlst)
    print(indlst)
